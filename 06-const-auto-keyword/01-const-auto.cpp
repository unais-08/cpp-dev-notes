 ...
 * -   **`const T* ptr;` (Pointer to `const` data):** The data pointed to cannot be modified through this pointer. The pointer itself *can* be changed to point to another location.
 * -   **`T* const ptr;` (`const` pointer to non-`const` data):** The pointer itself cannot be changed to point to another location. The data pointed to *can* be modified through this pointer.
 * -   **`const T* const ptr;` (`const` pointer to `const` data):** Neither the data pointed to nor the pointer itself can be modified.
 */
 ...
 * -   **`const T& ref;` (`const` reference):** The data referenced cannot be modified through this reference.
 ...
 * indicates that the function will not modify the state (member variables) of the object it's called on.
...
void demonstrateAutoBasic()
{
    ...
    std::cout << "cpp_string: " << cpp_string << ", type: " << typeid(cpp_string).name() << std::endl;
}
 ...
 * -   `auto` alone drops `const` and reference qualifiers. To preserve them, use `auto&` or `const auto&`.
 * -   **`auto&`:** Deduce the type and make it a reference. Preserves `const` if the initializer is `const`.
 * -   **`const auto&`:** Deduce the type and make it a `const` reference. Always results in a `const` reference.
 */
 ...
 * -   **No `auto` for function parameters (yet):** `void func(auto x) {}` is not allowed (except in C++20 for lambdas).
 * -   **No `auto` for return types (direct):** `auto func() { return 10; }` is allowed (C++14 onwards).
 * -   **Array Decay:** `auto arr = {1, 2, 3};` deduces `std::initializer_list<int>`, not an array.
 * `auto arr[] = {1, 2, 3};` is not allowed.
 * -   **Readability:** While `auto` reduces verbosity, sometimes explicit types improve clarity for complex logic. Use judiciously.
 */
