 ...
 * -   **Attributes (Member Variables):** The data or properties that objects of this class will possess.
 * -   **Behaviors (Member Functions):** The actions or operations that objects of this class can perform.
 * -   A class itself is not an object; it's a definition. You create **objects** (also called instances)
 * from a class. Many objects can be created from the same blueprint.
 */
...
public: // Access specifier (explained in Section 4)
    ...
    void bark()
    {
        std::cout << "2.1 " << name << " says Woof!" << std::endl;
    }
 ...
 * -   Each object has its own set of member variables, but they share the same member functions (methods).
 */
 ...
 * -   They enforce **encapsulation**, a core OOP principle that bundles data with the methods that operate on that data, and restricts direct access to some of the object's components.
 * -   **`private`:** Members declared `private` are accessible only from within the class itself (by its own member functions). They cannot be accessed directly from outside the class. This is the default access level if not specified.
 * -   **`public`:** Members declared `public` are accessible from anywhere, both inside and outside the class. These are typically the interface through which users interact with the object.
 * -   **`protected`:** (Will be covered in inheritance tutorials) Members declared `protected` are accessible from within the class itself and by derived classes (classes that inherit from it).
 */
...
public: // Public interface
    ...
    BankAccount(std::string accNum, double initialBalance)
    {
        ...
        std::cout << "4.1 Account " << accountNumber << " created with balance $" << balance << std::endl;
    }
...
void demonstrateAccessSpecifiers()
{
    ...
    std::cout << "Current balance for " << myAccount.getAccountNumber() << ": $" << myAccount.getBalance() << std::endl;
...
public:
    ...
    void display() const
    {
        std::cout << "5.1.1 Point: (" << x << ", " << y << ")" << std::endl;
    }
};
...
public:
    ...
    { // Parameterized Constructor
        length = l;
        width = w;
        std::cout << "5.2.1 Rectangle created: Length=" << length << ", Width=" << width << std::endl;
    }
...
public:
    ...
    { // Constructor with radius and color
        std::cout << "5.3.1 Circle created with radius " << radius << ", color " << color << "." << std::endl;
    }
    void display() const
    {
 ...
 * is about to be destroyed (goes out of scope, or `delete` is called on a dynamically allocated object).
 ...
 * -   **Key Uses:**
 * -   **Distinguishing between member variables and function parameters** when they have the same name.
...
public:
    ...
    void honk() const
    {
        std::cout << "8.1 " << brand << " goes Honk!" << std::endl;
    }
};
